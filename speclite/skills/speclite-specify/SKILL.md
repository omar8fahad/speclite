...
metadata:
  author: "speclite"
  source: "commands/speclite.specify.md"
---
...
4. If `specs/PROJECT_MAP.md` exists (the optional Project Map has been built), skim it -
   especially "Where to Look" and "Key Components/Domains" - before drafting. It saves
   rediscovering the codebase from scratch and helps this spec use the project's existing
   terminology. Not required if it doesn't exist yet.

5. Draft the spec into `SPEC_FILE` following `spec-template.md`'s structure:
...
6. **Clarify** - read `.speclite/templates/clarify-taxonomy.md` for the full rules, then run the
...
- [ ] `spec.md` written with no unresolved `[NEEDS CLARIFICATION]` markers (or the user
      explicitly accepted the risk of proceeding with one) - this is speclite's completion
      signal for this phase, read directly by `status`, no separate log involved
