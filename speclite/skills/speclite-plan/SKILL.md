...
metadata:
  author: "speclite"
  source: "commands/speclite.plan.md"
---
...
2. Read `SPEC_FILE` and everything listed in `REFERENCES` (grouped by subfolder - `PRD/`,
   ...
   at `.speclite/memory/principles.md`, if it exists. If `specs/ARCHITECTURE_MAP.md` and
   `specs/FILE_INDEX.md` exist (the optional Project Map has been built), check them too - they
   help avoid reinventing a pattern or module boundary that already exists.
...
- [ ] Pre-Implementation Checklist fully checked, or unchecked items explicitly called out to
      the user with a reason - this is speclite's completion signal for this phase, read
      directly by `status`, no separate log involved
